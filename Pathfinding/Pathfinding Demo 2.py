...
class ItzThetaStarPathing:
    ...
    def theta_star(self, goal, start = None, max_threads=4, step_size=1, demo = False): #Method for the Theta# implementation
        ...
        start = tuple(i*10 for i in start) #Work in a factor of ten
        goal = tuple(i*10 for i in goal) #Work in a factor of ten
        tol = 0.2
        while tol <= 1:
            k = 0
            ...
            with ThreadPoolExecutor(max_workers=max_threads) as executor:
                while open_set and k < 5000: #Loop for as long as their are nodes to be explored
                    k+=1
                    ...
                    if self.heuristic(current_node.position, goal) <= tol: #Check if final node is within tolerance around goal to counteract grid alignment errors
                        ...
                        while current_node: #Write the node tree to a list to be used
                            ...
                            if path[-1] != goal:
                                path.append(goal)
                        path = [tuple(p/10 for p in i) for i in path]
                    ...
                    for neighbour_pos in neighbours: #For all neighbours, check collisions
                        ...
                        local_current = tuple(i/10 for i in current_node.position)
                        local_neighbour = tuple(i/10 for i in neighbour_pos) #Divide coordinates by 10 before submitting for collision checks so that they are in the local frame of reference
                        futures[executor.submit(self.collision_check_demo, local_current, local_neighbour)] = neighbour_pos
    ...
    def plot_path(self,path, start, goal, col='blue'):
        ...
        if path:
            ...
            ax.scatter(path[:, 0], path[:, 1], path[:, 2], color=col, linewidth=2, label='Path')
...
if __name__ == "__main__":
    ...
    #print(path)
    #print(type(path[0]))
    ref_path = theta.refined_theta_star(goal_point, start_point, max_threads=4, fn = True, demo = True)
    #print(ref_path)
