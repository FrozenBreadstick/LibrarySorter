...
class LinearUR3(DHRobot3D):   
    ...
    def q(self, new_q):
        ...
        #If there is an active book, update it's base
        ...
        #     self.active.T = SE3(end_effector_pose) #Update the base of the gripper with the movement of the end effector
    ...
    def pickbook(self, book, n, gripper:float = None): #Function to pick up book from book's location
        logging.info(f"Picking book at position: \n{book}")
        #Hover above the book
        self.goto((book * SE3(0,0,0.4)), 100, n, gripper)
        ...
        #Attach book for movement
        self.activebook = book

    def dropbook(self, pos, n): #Function to drop book in target position
        logging.info(f"Dropping book in wall position: \n{pos}")
        #Place book in drop position, AlignXYZ is ideal as it ensure the books do not hit one another while being placed
        self.alignxyz((pos * SE3(0,0,0.315)), n)
        
        # Drop the book and retract gripper
        self.activegripper.open(1)
        self.activebook = None
        self.goto((self.fkine(self.q) * SE3(0,0,0.08)), 30, n)

        # Open gripper to pick up next book
        self.activegripper.open(0)

    def resetpos(self, n): #A function to bring the arm back to a common outstretched state to avoid crumpling while grabbin books
